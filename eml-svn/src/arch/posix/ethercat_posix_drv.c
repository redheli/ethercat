...
static BOOL ec_posix_txandrx_once(struct EtherCAT_Frame * frame, struct netif * ni) 
{
  ...
  if (handle < 0) {
    ec_log(EC_LOG_FATAL, "low_level_txandrx: send failed \n");
    return FALSE;
  }
  ec_log(EC_LOG_FATAL, "low_level_txandrx: receving \n");
...
static BOOL ec_rtdm_txandrx(struct EtherCAT_Frame * frame, struct netif * netif) {
  ...
  ec_log(EC_LOG_FATAL, "low_level_txandrx: failed %d timess: Giving up\n", MAX_TRIES_TX);
	return FALSE;
}
